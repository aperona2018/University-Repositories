...
public class ClientChat{
	public static void main(String args[]){
		try{
			...
			InterfazClient client = new ChatClient(nombre);
			InterfazClient c = (InterfazClient) Naming.lookup("rmi://localhost/ABC");
			System.out.println("Estás en un chat con: " + c.getName());
			
			ChatServer s = new ChatServer(nombre);
			// Creación de registro:
			LocateRegistry.createRegistry (8888); // puerto
	
	
			Naming.rebind("rmi://localhost/DEF", s); // nombre del stub
